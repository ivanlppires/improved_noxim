		...
		for (int j = 0; j < DIRECTIONS + 2; j++)
		{
			...
			if (!buffer[i].IsEmpty())
			{
				...
				if (flit.flit_type == FLIT_TYPE_HEAD)
				{
					...
					//~ cout << "\t- Enviarei pelo link " << o << "." << endl;


					LOG << " checking reservation availability of direction "
						<< o << " for flit " << flit << endl;
			...
			if (!buffer[i].IsEmpty())
			{
				...
				if (o != NOT_RESERVED)
				{
					if (current_level_tx[o] == ack_tx[o].read())
					{
						...
						//~ Writes in each flit the next hop
						flit.nexthop = -1;
						if (o != DIRECTION_LOCAL)
						{
							int nexthop = -1;
							AdmissibleOutputs outputs = routing_table.getAdmissibleOutputs(i,
																						   flit.
																						   dst_id);
							for (AdmissibleOutputs::iterator it = outputs.begin();
								 it != outputs.end(); it++)
							{
								const LinkId & out_link = *it;
								nexthop = out_link.second;
							}

							flit.nexthop = nexthop;
						}
						...
						if (o == DIRECTION_LOCAL)
						{
							//~ if (flit.flit_type == FLIT_TYPE_TAIL) cout << "\t- Rota Local" << endl;
							LOG << "\tConsumed flit " << flit << endl;
							stats.receivedFlit(sc_time_stamp().to_double() /
											   GlobalParams::clock_period_ps, flit);
...
vector < int >Router::routingFunction(const RouteData & route_data)
{
	if (GlobalParams::use_winoc)
	{
		...
		int nexthop = -1;
		AdmissibleOutputs outputs = routing_table.getAdmissibleOutputs(route_data.dir_in,
																	   route_data.dst_id);
		for (AdmissibleOutputs::iterator it = outputs.begin(); it != outputs.end(); it++)
		{
			const LinkId & out_link = *it;
			nexthop = out_link.second;
		}

		if (hasRadioHub(local_id) && hasRadioHub(nexthop) && !sameRadioHub(local_id, nexthop))
		{
...
int Router::route(const RouteData & route_data)
{
	...
	return selectionFunction(candidate_channels, route_data);
	 ...
	 void Router::NoP_report() const const const const
	 {
		 NoP_data NoP_tmp;
		 LOG << "NoP report: " << endl;
...
//---------------------------------------------------------------------------
	 ...
	 int Router::NoPScore(const NoP_data & nop_data, const vector < int >&nop_channels) const const const const
	 {
		 int score = 0;
...
int Router::selectionFunction(const vector < int >&directions, const RouteData & route_data)
{
...
unsigned int Router::getFlitsCount()
{
	...
	return count;
	 ...
	 int Router::reflexDirection(int direction) const const const const
	 {
	 ...
	 int Router::getNeighborId(int _id, int direction) const const const const
	 {
		 ...
		 case DIRECTION_NORTH:
			 ...
			 my_coord.y--;
			 break;
			 case DIRECTION_SOUTH:if (my_coord.y == GlobalParams::mesh_dim_y - 1)
				 return NOT_VALID;
			 my_coord.y++;
			 break;
			 case DIRECTION_EAST:if (my_coord.x == GlobalParams::mesh_dim_x - 1)
				 return NOT_VALID;
			 my_coord.x++;
			 break;
			 case DIRECTION_WEST:if (my_coord.x == 0)
				 return NOT_VALID;
			 my_coord.x--;
			 break;
			 default:LOG << "Direction not valid : " << direction;
			 assert(false);
		 }
