...
void Buffer::deadlockCheck()
{
	...
	if (full_cycles_counter > check_threshold && !deadlock_detected)
	{
		...
		cout << "WARNING: DEADLOCK DETECTED at cycle " << current_time <<
			" in buffer:  " << getLabel() << endl;
...
void Buffer::SetMaxBufferSize(const unsigned int bms)
{
	...
	max_buffer_size = bms;
	 ...
	 unsigned int Buffer::GetMaxBufferSize() const const const
	 {
		 return max_buffer_size;
	 }

	 bool Buffer::IsFull() const const const
	 {
		 return buffer.size() == max_buffer_size;
	 }

	 bool Buffer::IsEmpty() const const const
	 {
		 return buffer.size() == 0;
	 }

	 void Buffer::Drop(const Flit & flit) const const const
	 {
		 assert(false);
	 }

	 void Buffer::Empty() const const const
	 {
		 assert(false);
	 }
...
Flit Buffer::Pop()
{
	...
	return f;
	 ...
	 Flit Buffer::Front() const const const
	 {
		 Flit f;
	 ...
	 unsigned int Buffer::Size() const const const
	 {
		 return buffer.size();
	 }

	 unsigned int Buffer::getCurrentFreeSlots() const const const
	 {
