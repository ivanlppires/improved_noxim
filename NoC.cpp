// o roteador wireless só envia um por vez
/*
...
void NoC::buildMesh()
{
		 ...
		 GlobalParams::hub_configuration.begin(); it != GlobalParams::hub_configuration.end(); ++it)
	{
			 ...
			 iit != hub_config.txChannels.end(); ++iit)
		{
			...
			//LOG << "Binding " << hub[hub_id]->name() << " to txChannel " << channel_id << endl;
			hub[hub_id]->setFlitTransmissionCycles(channel
												   [channel_id]->getFlitTransmissionCycles
												   (), channel_id);
		}
		...
		{
			cerr <<
				" WARNING: Power model currently not supporting multi-channel per hub, using default_tx_energy"
				<< endl;
		}
		...
		// tx/tx buffer depth
		assert(GlobalParams::hub_configuration[hub_id].rxBufferSize ==
			   GlobalParams::hub_configuration[hub_id].txBufferSize);

		hub[hub_id]->power.configureHub(GlobalParams::flit_size,
										GlobalParams::hub_configuration
										[hub_id].toTileBufferSize,
										GlobalParams::hub_configuration
										[hub_id].fromTileBufferSize,
	...
	// traffic trace - transmitted and eof
	GlobalParams::trace_transmitted = new int[GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
	GlobalParams::trace_eof = new bool[GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
	GlobalParams::finalized = new int *[GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];

	for (int i = 0; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y; i++)
	{
		GlobalParams::trace_transmitted[i] = 0;
		GlobalParams::trace_eof[i] = false;
		GlobalParams::finalized[i] = new int[GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
	}
	...
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++)
	{
		for (int i = 0; i < GlobalParams::mesh_dim_x; i++)
		{
			...
			// traffic trace - Check for traffic trace availability
			t[i][j]->pe->traffic_trace =
				new GlobalTrafficTrace(GlobalParams::traffic_trace_filename.c_str(),
									   t[i][j]->pe->local_id);

			// Needed to choose destination
	...
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++)
	{
	 ...
	 Tile *NoC::searchNode(const int id) const const const const
	 {
		 for (int i = 0; i < GlobalParams::mesh_dim_x; i++)
			 for (int j = 0; j < GlobalParams::mesh_dim_y; j++)
