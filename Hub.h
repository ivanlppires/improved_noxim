...
SC_MODULE(Hub)
{
	...
	Hub(sc_module_name nm, int id, TokenRing * tr):sc_module(nm)
	{
		...
		{
			buffer_from_tile[i].SetMaxBufferSize(GlobalParams::
												 hub_configuration[local_id].fromTileBufferSize);
			buffer_to_tile[i].SetMaxBufferSize(GlobalParams::
											   hub_configuration[local_id].toTileBufferSize);
		...
		for (unsigned int i = 0; i < txChannels.size(); i++)
		{
			...
			init[txChannels[i]]->
				buffer_tx.SetMaxBufferSize(GlobalParams::hub_configuration[local_id].txBufferSize);
			...
			token_ring->attachHub(txChannels[i], local_id,
								  current_token_holder[txChannels[i]],
		...
		for (unsigned int i = 0; i < rxChannels.size(); i++)
		{
			...
			target[rxChannels[i]]->
				buffer_rx.SetMaxBufferSize(GlobalParams::hub_configuration[local_id].rxBufferSize);
