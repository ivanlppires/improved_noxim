...
int Hub::route(Flit & f)
{
	for (vector < int >::size_type i = 0;
		 i < GlobalParams::hub_configuration[local_id].attachedNodes.size(); i++)
	{
...
void Hub::rxPowerManager()
{
	...
	{
		if (!buffer_to_tile[port].IsEmpty() || !antenna2tile_reservation_table.isAvailable(port))
...
void Hub::txPowerManager()
{
	for (unsigned int i = 0; i < txChannels.size(); i++)
	{
		...
		if (!init[i]->buffer_tx.IsEmpty() || !tile2antenna_reservation_table.isAvailable(i))
		{
			power.leakageAntennaBuffer();
...
void Hub::txRadioProcessTokenPacket(int channel)
{
		...
		&& flag[channel]->read() != RELEASE_CHANNEL)
	{
		if (!init[channel]->buffer_tx.IsEmpty())
		{
			...
			//flag[channel]->write(HOLD_CHANNEL);
			LOG << "Requesting transmission transmission of flit " << flit <<
				" on channel " << channel << endl;
			init[channel]->start_request_event.notify();

			if (is_tail)
			{
				LOG << "TOKEN_PACKET: tail sent " << flit <<
					", releasing token for channel " << channel << endl;
		...
		else
		{
			...
			{
				LOG <<
					"TOKEN_PACKET: Buffer_tx empty and no trasmission in progress: releasing token for channel "
					<< channel << endl;
				flag[channel]->write(RELEASE_CHANNEL);
			}
			else
				LOG <<
					"TOKEN_PACKET: Buffer_tx empty, but trasmission in progress: holding token for channel "
					<< channel << endl;
...
void Hub::txRadioProcessTokenHold(int channel)
{
	...
	if (current_token_holder[channel]->read() == local_id)
	{
		if (!init[channel]->buffer_tx.IsEmpty())
		{
			...
			{
				LOG <<
					"TOKEN_HOLD policy: Not enough token expiration time for sending channel "
					<< channel << endl;
			}
			else
			{
		...
		{
			LOG <<
				"TOKEN_HOLD policy: nothing to transmit, holding token for channel "
				<< channel << endl;
		}
	}
}
	...
	for (int i = 0; i < num_ports; i++)
	{
		if (!buffer_to_tile[i].IsEmpty())
		{
			...
			if (current_level_tx[i] == ack_tx[i].read())
			{
				...
				LOG << "Flit " << flit << " moved from buffer_to_tile[" << i <<
					"] to signal flit_tx[" << i << "] " << endl;
			...
			{
				LOG << "Flit " << flit << " cannot move from buffer_to_tile["
					<< i << "] to signal flit_tx[" << i << "] " << endl;
			}
		}
	}
	...
	for (unsigned int i = 0; i < rxChannels.size(); i++)
	{
		...
		if (!(target[channel]->buffer_rx.IsEmpty()))
		{
			...
			if (!buffer_to_tile[r[i]].IsFull())
			{
				...
				LOG << "Moving flit  " << received_flit <<
					" from buffer_rx to buffer_to_tile, port " << r[i] << endl;
...
void Hub::tileToAntennaProcess()
{
	...
	//~ Ivan & Zanata moved it to the constructor
	...
	for (int j = 0; j < num_ports; j++)
	{
		...
		if (!buffer_from_tile[i].IsEmpty())
		{
			...
			if (flit.flit_type == FLIT_TYPE_HEAD)
			{
				...
				//~ int channel = selectChannel(local_id,tile2Hub(flit.dst_id)); // ORIGINAL
				int channel = selectChannel(local_id, tile2Hub(flit.nexthop));	
				...
				{
					LOG << "Reservation:  wireless channel " << channel <<
						" not available ..." << endl;
				}
		...
		if (!buffer_from_tile[i].IsEmpty())
		{
			...
			if (channel != NOT_RESERVED)
			{
				if (!(init[channel]->buffer_tx.IsFull()))
				{
					...
					LOG << "Flit " << flit << " moved from buffer_from_tile["
						<< i << "] to buffer_tx[" << channel << "] " << endl;
				}
				else
				{
					LOG << "Buffer Full: Cannot move flit " << flit <<
						" from buffer_from_tile[" << i << "] to buffer_tx[" <<
						channel << "] " << endl;
			...
			{
				LOG << "Forwarding: No channel reserved for port direction " <<
					i << ", flit " << flit << endl;
			}
		}
