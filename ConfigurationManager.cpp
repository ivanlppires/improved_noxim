...
void loadConfiguration()
{
	...
	GlobalParams::traffic_trace_flit_headtail_size =
		config["traffic_trace_flit_headtail_size"].as < int >();
}
...
void showHelp(char selfname[])
{
	cout << "Usage: " << selfname << " [options]" << endl
		...
		<< endl << "\t-dimx N\t\tSet the mesh X dimension" << endl <<
		...
		endl << "\t\tRANDOM\t\tRandom selection strategy" << endl <<
		"\t\tBUFFER_LEVEL\tBuffer-Level Based selection strategy" << endl <<
		"\t\tNOP\t\tNeighbors-on-Path selection strategy" << endl <<
		"\t-pir R TYPE\tSet the packet injection rate R [0..1] and the time distribution TYPE"
		<< endl << "\t\t\twhere TYPE is one of the following:" << endl <<
		...
		<< endl << "\t\trandom\t\tRandom traffic distribution" << endl <<
		...
		endl << "\t\ttranspose1\tTranspose matrix 1 traffic distribution" <<
		endl << "\t\ttranspose2\tTranspose matrix 2 traffic distribution" <<
		endl << "\t\tbitreversal\tBit-reversal traffic distribution" << endl <<
		...
		<< endl << "\t-warmup N\tStart to collect statistics after N cycles" <<
		endl <<
		"\t-seed N\t\tSet the seed of the random generator (default time())" <<
		endl << "\t-detailed\tShow detailed statistics" << endl <<
...
void showConfig()
{
	cout << "Using the following configuration: " << endl
		...
		<< "- selection_strategy = " << GlobalParams::selection_strategy <<
		endl << "- packet_injection_rate = " <<
		GlobalParams::packet_injection_rate << endl <<
		...
		endl << "- clock_period = " << GlobalParams::clock_period_ps << "ps" <<
		endl << "- simulation_time = " << GlobalParams::simulation_time << endl
...
void checkConfiguration()
{
	...
	if (GlobalParams::packet_injection_rate <= 0.0 || GlobalParams::packet_injection_rate > 1.0)
	{
	...
	{
		if (GlobalParams::hotspots[i].first >= GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)
		{
			cerr << "Error: hotspot node " << GlobalParams::hotspots[i].
				first << " is invalid (out of range)" << endl;
			exit(1);
		}
	...
	if (GlobalParams::traffic_trace_flit_headtail_size < 1 &&
		GlobalParams::traffic_distribution == TRAFFIC_TRACE)
	{
		cerr << "Error: the size of flit head/tail must be positive" << endl;
		exit(1);
	}
		...
		for (int i = 1; i < arg_num; i++)
		{
			...
			else if (!strcmp(arg_vet[i], "-pir"))
			{
				...
				else if (!strcmp(distribution, "pareto"))
				{
					...
					GlobalParams::probability_of_retransmission =
						GlobalParams::packet_injection_rate * pow((1 - r), (1 / Aoff - 1 / Aon));
				}
			...
			else if (!strcmp(arg_vet[i], "-traffic"))
			{
				...
				{				// traffic trace
					GlobalParams::traffic_distribution = TRAFFIC_TRACE;
					GlobalParams::traffic_trace_filename = arg_vet[2 + i];
				}
