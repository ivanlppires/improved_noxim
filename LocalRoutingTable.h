...
class LocalRoutingTable
{
  ...
  public:
	...
	AdmissibleOutputs getAdmissibleOutputs(const LinkId & in_link, const int destination_id);

	// Returns the set of admissible output channels for a destination
	// destination_id and a given input direction
	AdmissibleOutputs getAdmissibleOutputs(const int in_direction, const int destination_id);
