...
unsigned int GlobalStats::getWirelessPackets()
{
	...
	for (map < int, HubConfig >::iterator it =
		 GlobalParams::hub_configuration.begin(); it != GlobalParams::hub_configuration.end(); ++it)
	{
		int hub_id = it->first;
...
void GlobalStats::showStats(std::ostream & out, bool detailed)
{
	...
	int total_cycles = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
	int total_hpp =
		GlobalParams::total_headtail + GlobalParams::total_payload + GlobalParams::total_padding;
	int total_flits =
		ceil(ceil(total_hpp / (double)getReceivedPackets()) /
			 (double)GlobalParams::flit_size) * getReceivedPackets();

	out << endl;
	out << "% ** Data transmited **\tbyte" << endl;
	out << "# Total headtail:\t" << (GlobalParams::total_headtail / 8) << endl;
	out << "# Total payload:\t" << (GlobalParams::total_payload / 8) << endl;
	out << "# Total padding:\t" << (GlobalParams::total_padding / 8) << endl;
	out << "% Total h+p+p:\t\t" << total_hpp / 8 << endl;
	out << "%" << endl;
	out << "# Total transmit_flits:   " << total_flits << endl;

	out << endl;
	out << "% ** Data received ** " << endl;
	out << "# Total received_flits:   " << getReceivedFlits() << endl;
	out << "% Total received_packets: " << getReceivedPackets() << endl;
	out << "# Total executed_cycles:  " << total_cycles << endl;
	out << "% Total executed_time:    " << ceil(total_cycles /
												GlobalParams::clock_period_ps) << " ns" << endl;

	out << endl;
	out << "% ** Wireless usage **" << endl;
	out << "% Total Packets over wireless: " << getWirelessPackets() << endl;
	out << "% Total Flits over wireless:   " << GlobalParams::total_wirelessflits << endl;
	out << "% Total bits over wireless:    " <<
		GlobalParams::total_wirelessflits * GlobalParams::flit_size << endl;
	out << endl;
	...
	out << "% ** Average and Throughput ** " << endl;
	out << "% Average wireless utilization:           " << getWirelessPackets()
		/ (double)getReceivedPackets() << endl;
	out << "% Global average delay (cycles):          " << getAverageDelay() << endl;	//  avg_delay /= (double) total_packets;
...
void GlobalStats::showPowerManagerStats(std::ostream & out)
{
		 ...
		 GlobalParams::hub_configuration.begin(); it != GlobalParams::hub_configuration.end(); ++it)
	{
		...
		out << "\t" << hub_id << "\t" << std::fixed << (double)h->total_ttxoff_cycles /
			total_cycles << "\t";
		 ...
		 GlobalParams::hub_configuration.begin(); it != GlobalParams::hub_configuration.end(); ++it)
	{
		...
		out << "\t" << hub_id << "\t" << std::fixed << (double)h->total_sleep_cycles /
			total_cycles << "\t";
		...
		for (map < int, int >::iterator i =
			 h->buffer_to_tile_poweroff_cycles.begin();
		...
		out << (double)s / h->buffer_to_tile_poweroff_cycles.size() /
			total_cycles << "\t" << bttoff_str << endl;
	}
